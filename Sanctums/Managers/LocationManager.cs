...
public class LocationManager
{
    ...
    internal static void PieceAwakePatch(Piece __instance)
    {
        ...
        if (name != "KingSanctum") return;
        if (__instance.GetComponent<Behaviors.Sanctum>()) return;
        __instance.gameObject.AddComponent<Behaviors.Sanctum>();
    }
    private static void SetupLocations_Prefix(ZoneSystem __instance)
    {
        ...
        foreach (LocationData? location in m_locations.Values)
        {
            ...
            {
                SanctumsPlugin.SanctumsLogger.LogDebug(data.m_prefabName + " is not valid");
            }
        }
        __instance.m_locations.AddRange(locations);
        ZLog.Log($"Added {locations.Count} locations from Sanctums");
    }
    ...
    public class LocationData
    {
        ...
        public LocationData(string name, AssetBundle bundle, string sanctum = "")
        {
            ...
            m_sanctum = sanctum;
            if (!m_sanctum.IsNullOrWhiteSpace())
            {
                SanctumManager.AddPrefabToSearch(m_sanctum);
            }
            SanctumsPlugin.m_assetLoaderManager.AddAsset(prefab, out AssetID assetID);
            AssetID = assetID;
            m_data.m_prefabName = prefab.name;
            m_locations[prefab.name] = this;
            
            SanctumsPlugin.SanctumsLogger.LogDebug("Registered location: " + name);
        }
        ...
        public ZoneSystem.ZoneLocation GetLocation()
        {
            return new ZoneSystem.ZoneLocation()
            {
                ...
                m_prefab = SanctumsPlugin.m_assetLoaderManager.GetSoftReference(AssetID),
